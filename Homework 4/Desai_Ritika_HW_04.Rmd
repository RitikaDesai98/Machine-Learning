...
**The Average of the vector 'x' and the Maximum Likelihood Estimate is found to be the same**
...
$$
\log[p \left( \mathbf{x} \mid \mu \right)] = log \left(\mu ^ {x_n} \left(1 - \mu \right) ^ \left(1 - x_n \right) \right)
$$
$$
\log[p \left( \mathbf{x} \mid \mu \right)] =  \left(log \left(\mu ^ {x_n}\right) + log \left(\left(1 - \mu \right) ^ \left(1 - x_n \right) \right) \right)
$$


$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \left(x_n \cdot log\left( \mu \right) + \left(1 - x_n \right) \cdot log \left(1 - \mu \right) \right) 
$$
...
{
   vals <- c()
   vals <- (xobs * log(prob)) + ((1 - xobs) * log(1 - prob))
   return (vals)
}
```
...
```{r}
log_bernoulli_pmf(x[1], 0.25)
```
  
```{r}
log_bernoulli_pmf(x[2], 0.25)
```

```{r}
log_bernoulli_pmf(x[3], 0.25)
```

```{r}
log_bernoulli_pmf(x[4], 0.25)
```

```{r}
log_bernoulli_pmf(x[5], 0.25)
```
...
```{r}
length(log_bernoulli_pmf(x[1:5], 0.25))
```

**The length of the returned result is 5. The values returned are the same as previously when the function was called separately.**
...
$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \log \left(\prod_{n=1} ^ N \left(\mu^{x_n}\left(1 - \mu \right)^\left( {1-x_n} \right) \right) \right)
$$

$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \sum_{n=1} ^ N \left(log \left [ \mu^{x_n}\left(1 - \mu \right)^\left( {1-x_n} \right) \right] \right)
$$

$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \sum_{n=1} ^ N \left(log \left [ \mu^{x_n} \right] + log \left[ \left(1 - \mu \right)^\left( {1-x_n} \right) \right] \right)
$$

$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \sum_{n=1} ^ N \left(x_n log \left [ \mu \right] + \left( {1-x_n} \right) log \left[ 1 - \mu \right] \right)
$$
$$
\log[p \left( \mathbf{x} \mid \mu \right)] = \sum_{n=1} ^ N \left( x_n log \left [ \mu \right]\right) + \sum_{n=1} ^ N \left(\left( {1-x_n} \right) log \left[ 1 - \mu \right] \right)
$$
$$
\log[p \left( \mathbf{x} \mid \mu \right)] = log \left[\mu \right] \sum_{n=1} ^ N \left(x_n \right) + log \left[1-\mu \right]\sum_{n=1} ^ N \left(1-x_n \right)
$$

$$
\log[p \left( \mathbf{x} \mid \mu \right)] = log \left[\mu \right] \times M + log\left[1 - \mu \right] \times \left(N - M \right)
$$
...
```{r}
log_likelihood <- (log(0.25) * sum(x)) + (log(1 - 0.25)) * (length(x) - sum(x))
log_likelihood
```
...
```{r}
xa_results %>% ggplot(mapping = aes(x = mu, y = log_lik)) + geom_line()
```
...
```{r}
xa_results %>% ggplot(mapping = aes(x = mu, y = log_lik)) + geom_line() + geom_vline(xintercept = mle)
```
...
**The log likelihood is maximum near the mle and the curve decreases around the mle on either side.**
...
$$
$$
\log \left( p \left( H \mid AB, \mu \right) \right) \propto log \left(\mu^H\right) + log \left[\left(1 - \mu \right)^{AB-H} \right]
$$
$$
...
tibble::tibble(
  ...
  mutate(log_lik = player_hits*log(mu) + (player_atbats-player_hits)*log(1-mu)) %>% ggplot(mapping = aes(x = mu, y = log_lik)) + geom_line(size = 1.1) + geom_vline(xintercept = mle, size = 1, linetype = 'dashed', color = 'red')
```
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(player_hits, player_atbats, mu, log = TRUE)) %>% ggplot(mapping = aes(mu, y = log_lik)) + geom_line(size = 1.1) + geom_vline(xintercept = mle, size = 1, linetype = 'dashed', color = 'red')
```
...
**The values at which the figure is plotted has changed but the shape remains same**
...
**For the uniform set, trials are a_uniform + b_uniform. For the inform set, trials are a_inform + b_inform**
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_uniform, shape2 = b_uniform)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_inform, shape2 = b_inform)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
$$
$$
\log \left( p \left( m \mid N, \mu \right) \right) \propto log \left(\mu^m\right) + log\left[ \left(1 - \mu \right)^{N-M} \right]
$$
...
$$
$$
\log \left( p \left( m \mid N, \mu \right) \right) \propto log \left(\mu^{a-1}\right) + log\left[ \left(1 - \mu \right)^{b-1} \right]
$$
$$
\log \left( p \left( m \mid N, \mu \right) \right) \propto \left(a-1 \right) \cdot log \left(\mu\right) + \left(b-1 \right) \cdot log\left(1 - \mu \right)
$$
...
$$ 
\frac {\delta}{\delta \mu} \{log \left(p\left(\mu \mid m, N \right) \right) \} \propto \frac {\delta}{\delta \mu} \{a_{new} log\left(\mu \right) + b_{new}log(1 - \mu) \}
$$
$$
\frac {\delta}{\delta \mu} \{log \left(p\left(\mu \mid m, N \right) \right) \} \propto \frac {\delta}{\delta \mu} \left[a_{new} log\left(\mu \right)\right] + \frac {\delta}{\delta \mu} \left[b_{new}log(1 - \mu) \right]
$$
$$
\frac {\delta}{\delta \mu} \{log \left(p\left(\mu \mid m, N \right) \right) \} \propto a_{new} \frac {\delta}{\delta \mu} \left[log\left(\mu \right)\right] + b_{new} \frac {\delta}{\delta \mu} \left[log(1 - \mu) \right]
$$
$$
...
$$
a_{new} - (1 - \mu_{MAP}) = b_{new}(\mu_{MAP})
$$
$$
a_{new} - \left(a_{new} \cdot \mu_{MAP} \right) = (b_{new} \cdot \mu_{MAP})
$$
$$
...
b_uniform_new <- b_uniform + (player_atbats - player_hits)
```
  
```{r}
a_inform_new <- a_inform + player_hits
b_inform_new <- b_inform + (player_atbats - player_hits)
```
...
```{r}
a_uniform_mean <- a_uniform_new / (a_uniform_new + b_uniform_new)
```

```{r}
a_inform_mean <- a_inform_new / (a_inform_new + b_inform_new)
```

```{r}
uniform_mode <- max(dbeta(mu_grid, shape1 = a_uniform_new, shape2 = b_uniform_new))
inform_mode <- max(dbeta(mu_grid, shape1 = a_inform_new, shape2 = b_inform_new))
```

```{r}
uniform_p_5 <- qbeta(p = 0.05, shape1 = a_uniform_new, b_uniform_new)
uniform_p_95 <- qbeta(p = 0.95, shape1 = a_uniform_new, b_uniform_new)
```

```{r}
inform_p_5 <- qbeta(p = 0.05, shape1 = a_inform_new, b_inform_new)
inform_p_95 <- qbeta(p = 0.95, shape1 = a_inform_new, b_inform_new)
```

```{r}
data.frame(Name = c("Mean", "Mode", "5th Percentile", "95th Percentile"),
           Uniform = c(a_uniform_mean, uniform_mode, uniform_p_5, uniform_p_95),
           Inform = c(a_inform_mean, inform_mode, inform_p_5, inform_p_95))
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_uniform_new, shape2 = b_uniform_new)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_inform_new, shape2 = b_inform_new)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
**The values of a and b increase and the width of the beta distribution curve becomes smaller. Beta distribution gets more confident about the density being in that interval**
...
```{r}
a_uniform_newer <- a_uniform + season_hits
b_uniform_newer <- b_uniform + (season_atbats - season_hits)
```
  
```{r}
a_inform_newer <- a_inform + season_hits
b_inform_newer <- b_inform + (season_atbats - season_hits)
```
...
```{r}
a_uniform_mean_2 <- a_uniform_newer / (a_uniform_newer + b_uniform_newer)
```

```{r}
a_inform_mean_2 <- a_inform_newer / (a_inform_newer + b_inform_newer)
```

```{r}
uniform_mode_2 <- max(dbeta(mu_grid, shape1 = a_uniform_newer, shape2 = b_uniform_newer))
inform_mode_2 <- max(dbeta(mu_grid, shape1 = a_inform_newer, shape2 = b_inform_newer))
```

```{r}
uniform_p_5_2 <- qbeta(p = 0.05, shape1 = a_uniform_newer, b_uniform_newer)
uniform_p_95_2 <- qbeta(p = 0.95, shape1 = a_uniform_newer, b_uniform_newer)
```

```{r}
inform_p_5_2 <- qbeta(p = 0.05, shape1 = a_inform_newer, b_inform_newer)
inform_p_95_2 <- qbeta(p = 0.95, shape1 = a_inform_newer, b_inform_newer)
```

```{r}
data.frame(Name = c("Mean", "Mode", "5th Percentile", "95th Percentile"),
           Uniform = c(a_uniform_mean_2, uniform_mode_2, uniform_p_5_2, uniform_p_95_2),
           Inform = c(a_inform_mean_2, inform_mode_2, inform_p_5_2, inform_p_95_2))
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_uniform_newer, shape2 = b_uniform_newer)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = a_inform_newer, shape2 = b_inform_newer)) %>% ggplot(mapping = aes(x = mu, y = beta_pdf)) + geom_line()
```
...
**Smaller the sample size, more significant the influence of the prior and posterior. In the 6th question, the sample size is small and when we add the it to the prior and posterior, it made a difference in the uniform and inform.**
**Sample size being bigger, adding the prior and posterior didn't make that much of a difference. The curve is still similar with a slightly different mode**
