...
**We have 7 colors and each color has a boxplot as per the price for that color. The line in the center of the box plot indicates the median price of that color. Either sides of the lines indicate the maximum and minimum price for the color. The outliers are at the top of the line giving an illusion of a thickened line.**
...
```{r, solution_02e}
 diamonds %>% ggplot(mapping = aes(x=color, y=price)) + geom_boxplot(aes(fill=carat>median(carat))) + facet_grid(clarity ~ cut) + theme(legend.position = "top")
```
...
```{r, solution_02f}
diamonds %>% filter(cut=="Ideal" & clarity == "IF") %>% ggplot(mapping = aes(x=color, y=price, fill=carat>median(diamonds$carat))) + geom_boxplot() + facet_grid(clarity ~ cut) + theme(legend.position = "top")
```
...
**The figure 2b gives us a boxplot of prices as per the color, whereas figure 2f gives us a conditioned boxplot based on cut = ideal, clarity = IF and if the carat > median(carat). We see that the diamonds which have carat>median(carat) are more expensive as compared to carat<median(carat).**
...
```{r, solution_03c}
mpg %>% ggplot(mapping=aes(x = displ, y = hwy)) + geom_point() + geom_smooth(method = lm)
```
...
```{r, solution_03d}
mpg %>% ggplot(mapping=aes(x = displ, y = hwy)) + geom_point() + geom_smooth(formula = y~x, method = lm)
```
...
```{r, solution_03e}
mpg %>% ggplot(mapping=aes(x = displ, y = hwy)) + geom_point() + geom_smooth(formula = y~x, method = lm, mapping = aes(color = drv))
```
...
```{r, solution_03f}
mpg %>% ggplot(mapping=aes(x = displ, y = hwy)) + geom_point() + geom_smooth(formula = y~x, method = lm, mapping = aes(group = drv))
```
...
**Yes it is significant as it is always negative.**

#### SOLUTION

```{r, solution_04b}
mod1 %>% coefplot::coefplot()
```
...
**Yes, all the variables are significant. 4 coefficients are displayed.**

#### SOLUTION

```{r, solution_04d}
mod2 %>% coefplot::coefplot()
```
