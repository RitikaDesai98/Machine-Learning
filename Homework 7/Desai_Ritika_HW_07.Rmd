...
```{r}
quad_data_big %>% tibble::rowid_to_column() %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(mapping = aes(color = rowid > 5), size = 3)
```
...
**The first column contains 1's because we created a fake input variable $x_{n,0}$.**
...
```{r}
sd_small <- sqrt(diag(covmat_quad_small_noprior))
sd_small
```

```{r}
sd_big <- sqrt(diag(covmat_quad_big_noprior))
sd_big
```
...
**The values would remain the same as the value of MAP of beta does not depend on sigma.**
...
```{r}
postmeans_quad_small_noprior[3] - 3*1
```
...
```{r}
postmeans_quad_big_noprior[3] - 3*1 
```
...
What do you think?  
**The quadratic feature is not significant for large data but it is significant for small data.**
...
```{r}
sqrt(diag(covmat_quad_small_inform))
```

```{r}
sqrt(diag(covmat_quad_big_inform))
```

**For big data, both inform and no-prior have the same sd. But for small data, the sd for inform is smaller than no-prior.**
...
```{r}
sqrt(diag(covmat_quad_small_vague))
```

```{r}
sqrt(diag(covmat_quad_big_vague))
```


**Yes the values of standard deviation of vague prior are similar to no-prior in both cases.**
...
What do you think?  
**The standard deviation of the regression coefficients is less when using informative prior compared to the vague prior for small data but it is pretty similar for big data** 
...
**No, the initial guess would not impact the results as it is a linear model and there is just one unique maximum **
...
set.seed(87123)
post_samples_quad <- generate_lm_post_samples(laplace_quad, ncol(info_quad$design_matrix), 2.5e3)
```
...
**Rows are the number of predictions or test points. Columns are random posterior samples of the beta parameter.**
...
mae_quad_post_01 <- sum(abs(error_quad_post_01)) / length(error_quad_post_01)

mae_quad_post_02 <- sum(abs(error_quad_post_02)) / length(error_quad_post_02)

mae_quad_post_03 <- sum(abs(error_quad_post_03)) / length(error_quad_post_03)
```

**We are averaging over the length of the error vector (30). The three MAE values are not the same. They are different because in each case the outcome is predicted using a particular value of beta for that case and therefor we get different error values and hence different MAE.**
...
Which function?  
**colMeans() can be used to get the average value as each column is the random sample of beta.**
...
**MAE_quad[3] and mae_quad_post_03 are the same but still getting FALSE as output.**
...
**The uncertainty in the beta creates the uncertainty in the mean trend.**
...
```{r solution_05a}
ggplot(mapping = aes(x = x, y = y)) + geom_point(data = train_df) + geom_point(data = test_df, color = "red")
```
...
**There are 150 rows in the training design matrix and 50 rows in test matrix.**
...
all_spline_mae_results %>% ggplot(mapping = aes(x = as.factor(J), y = mae)) + geom_boxplot(mapping = aes(fill = dataset)) + scale_fill_brewer(palette = "Set1")
```

Summarize the posterior samples on the MAE on the training and test sets by focusing on the posterior median MAE values.  

```{r, solution_05e_b, eval=FALSE}
all_spline_mae_results %>% ggplot(mapping = aes(x = J, y = mae)) + stat_summary(mapping = aes(color = dataset), geom = "line", fun = "median") + scale_fill_brewer(palette = "Set1")
```

Which degrees-of-freedom overfit?  
**Models with degrees-of-freedom higher than 9 are overfitting the data as the training MAE is very low and the test MAE is increasing.**
...
**According to the visualization, model 7 seems to be the best **
...
What do you think?  
**Yes, they are different. The train dataset concluded that models with higher dof are better. The evidence based concludes model 7 is the best. This is because evidence not only sees all the combinations of the parameters that allow the model to fit the data but also sees all the combinations of the parameters that cause the model to perform poorly. **
