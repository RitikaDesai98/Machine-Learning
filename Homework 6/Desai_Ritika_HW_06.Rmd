...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = avg_x, xsd = sd_x),
...
**The sample average and standard deviation is not similar to the posterior mode. The posterior uncertainty in $\mu$ and $\sigma$ based on the visualization is 99% on the values of $\mu$ are between 9 and 12.5 and `99%` of all values of $\sigma$ are between 2 and 6.25.  **
...
$$
Corresponding 1st derivative of the un-normalized log-posterior wrt $\mu$
$$
...
$$
So the 2nd derivative of the un-normalized log-posterior wrt $\mu$ is
$$
...
To check if that the mode does in fact correspond to the $\mu$ value associated with the maximum log-posterior density and not the minimum log-posterior density we need to check the value of the second derivative at $\mu_{MAP}$

Since we know that 
$$
\mu_{MAP} = \frac{ \frac{1}{\tau_0^2} \mu_0 + \frac{N}{\sigma^2} \bar{x}}{\frac{1}{\tau_0^2} + \frac{N}{\sigma^2}}
$$
And keeping the value of $\mu_{MAP}$ in the second derivative we get 
$$
...
$$
**Now the value of $\mu_{MAP}$ will be small compared to $\bar{x}$ so the first term will be positive. Even if value of $\mu_{MAP}$ is smaller than $\mu$, the value of first term will be greater than the second term as it is multiplied $N$. So overall the value of the second derivative will be negetive and hence the value of function at $\mu_{MAP}$ is local maximum. But as this is a Gaussian distribution and the Gaussian is unimodal it is the only maximum value. **
...
**We use Laplace approximation to approximate as a multivariate normal. The likelihood noise $/sigma$ has a natural lower bound of 0. A  gaussian distribution jas no such constraints and so allows all values from - infinity to + infinity. Therefore even though the prior exponential distribution respects the bound on sigma the lower bound would be violated if we apply the laplace approximation to approximate the posterior**
...
$$
log \left(p \left(\mu,\phi \mid \textbf{x} \right) \right) = \sum_{n=1}^N log \left(\mathrm{normal} \left(x_n \mid \mu, g^{-1} \left(\phi \right) \right) \right) + log \left(\mathrm{normal} \left(\mu \mid \mu_0, \tau_0 \right)\right) + log \left(\mathrm{Exp} \left(g^{-1} \left(\phi \right) \mid \lambda \right) \right) + log \left| \frac{d}{d\phi} \left(g^{-1} \left(\phi \right) \right) \right|
$$
...
cv_param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = avg_x, log_xsd = log(sd_x)),
...
**Yes the identified optimal parameter values and the Hessian matrices values are same. The count parameter was different. For the first initial guess the function was called 12 times and 6 gradients were calculated to get the optimal value whereas for the second initial guess the function was called 21 times and 11 gradients were calculated. So both optimization gives us the same result only the number of steps and the path taken to reach the optima parameter is different as our starting point is different.**  
...
```{r}
pnorm(avg_x, mean = laplace_result$mode[1], sd = laplace_result$mode[2])
```
