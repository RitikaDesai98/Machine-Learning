...
```{r}
quantile_25 = qnorm(0.25, mean = mu_0, sd = tau_0)
quantile_75 = qnorm(0.75, mean = mu_0, sd = tau_0)
```
...
**The interval between the two quantiles contains 50% of the probability mass**
...
$$
$$
\frac{N}{\sigma^2} = \frac{1}{\tau_{N}^{2}} - \frac{1}{\tau_{0} ^ {2}}
$$

$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_{N}^{2}} - \frac{1}{\tau_{0} ^ {2}} \right)
$$
...
```{r}
N_1 = sigma_1^2 * ((1/tau_n1^2) - (1/tau_0^2))
N_2 = sigma_1^2 * ((1/tau_n2^2) - (1/tau_0^2))
```
...
$$
$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_{N}^{2}} - \frac{1}{\infty} \right)
$$
$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_{N}^{2}} - \frac{1}{0} \right)
$$
$$
N = \left(\frac{\sigma^2}{\tau_{N}^{2}} \right)
$$
...
```{r}
N_3 = sigma_2^2 * ((1/tau_n3^2) - (1/tau_0_inf^2))
```
...
$$
p\left(\mu \mid \mathbf{x}, \sigma\right) \propto \prod_{n=1}^{N} \left[\exp \left( \frac{-1}{2\sigma^2} \left(x_n - \mu \right)^2 \right) \right] \cdot \exp \left(\frac{-1}{2\tau_0^2} \left(\mu - \mu_0 \right)^2 \right)
$$

$$
log \left[p\left(\mu \mid \mathbf{x}, \sigma\right)\right] \propto log \left(\prod_{n=1}^{N} \left[\exp \left( \frac{-1}{2\sigma^2} \left(x_n - \mu \right)^2 \right) \right] \cdot \exp \left(\frac{-1}{2\tau_0^2} \left(\mu - \mu_0 \right)^2 \right)\right)
$$
$$
log \left[p\left(\mu \mid \mathbf{x}, \sigma\right)\right] \propto log \left(\prod_{n=1}^{N} \left[\exp \left( \frac{-1}{2\sigma^2} \left(x_n - \mu \right)^2 \right) \right]\right) + log \left(\exp \left(\frac{-1}{2\tau_0^2} \left(\mu - \mu_0 \right)^2 \right)\right)
$$
$$
...
$$
\frac{\delta}{\delta \mu}log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \frac{\delta}{\delta \mu} \left(\frac{-1}{2\sigma^2} \sum_{n=1}^{N} \left[\left(x_n - \mu \right)^2 \right]\right) - \frac{\delta}{\delta \mu} \left(\frac{1}{2\tau_0^2} \left(\mu - \mu_0 \right)^2\right)
$$
$$
\frac{\delta}{\delta \mu}log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \frac{-1}{2\sigma^2} \sum_{n=1}^{N} \frac{\delta}{\delta \mu}\left(x_n - \mu \right)^2 - \frac{1}{2\tau_0^2} \frac{\delta}{\delta \mu} \left(\mu - \mu_0 \right)^2
$$
$$
\frac{\delta}{\delta \mu}log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \frac{1}{\sigma^2} \sum_{n=1}^{N}\left(x_n - \mu \right) - \frac{1}{\tau_0^2}  \left(\mu - \mu_0 \right)
$$
$$
\frac{\delta}{\delta \mu}log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \frac{1}{\sigma^2} \left(\sum_{n=1}^{N}x_n - \mu \sum_{n=1}^{N} 1 \right) - \frac{1}{\tau_0^2}  \left(\mu - \mu_0 \right)
$$
$$
\frac{\delta}{\delta \mu}log \left[p\left(\mu \mid \mathbf{x}, \sigma\right) \right] \propto \frac{1}{\sigma^2} \left(\bar{x} N - \mu N \right) - \frac{1}{\tau_0^2}  \left(\mu - \mu_0 \right)
$$
$$
...
$$
0 = \frac{N}{\sigma^2} \left (\bar{x} - \mu_{MAP} \right) - \frac{1}{\tau_0^2} \left(\mu_{MAP} - \mu_0 \right)
$$
$$
\frac{N}{\sigma^2} \left (\bar{x} - \mu_{MAP} \right) = \frac{1}{\tau_0^2} \left(\mu_{MAP} - \mu_0 \right)
$$
$$
\frac{N}{\sigma^2} \bar{x} - \frac{N}{\sigma^2} \mu_{MAP} = \frac{1}{\tau_0^2} \mu_{MAP} - \frac{1}{\tau_0^2} \mu_0
$$

$$
\frac{N}{\sigma^2} \bar{x} + \frac{1}{\tau_0^2} \mu_0 = \frac{1}{\tau_0^2} \mu_{MAP} + \frac{N}{\sigma^2} \mu_{MAP}
$$
...
**The expression is same as the posterior mean expression. It is because the distribution of the posterior will be Gaussian and for a Gaussian distribution mean = mode = median**
...
$$
p \left(\sigma \mid \mathbf{x}, \mu\right) \propto \prod_{n=1}^{N} \left( \frac{1}{\sigma \sqrt{2\pi}} \exp \left[\frac{-1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] \right) \cdot \lambda\exp \left(-\lambda \sigma \right)
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \log \left( \left(  \prod_{n=1}^{N} \frac{1}{\sigma \sqrt{2\pi}} \exp \left[\frac{-1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] \right) \cdot \lambda\exp \left(-\lambda \sigma \right) \right)
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto  \sum_{n=1}^{N} \log\left(   \frac{1}{\sigma \sqrt{2\pi}} \exp \left[\frac{-1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] \right)  + \log \left(\lambda\exp \left(-\lambda \sigma \right) \right)
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \sum_{n=1}^{N} \left[ \log \left(\frac{1}{\sigma \sqrt{2\pi}} \right) + \log \left(\exp \left(\frac{-1}{2\sigma^2} \left( x_n  - \mu \right)^2\right) \right) \right] + \log \left(\lambda \right) + \log \left(\exp \left(-\lambda\sigma\right) \right)
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \sum_{n=1}^{N} \left[- \log \left(2\pi\sigma^2 \right)^{\frac{1}{2}} - \frac{1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] + \log \left(\lambda \right) -\lambda\sigma
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \sum_{n=1}^{N} \left[- \log \left(2\pi \right)^{\frac{1}{2}} - \log \left(\sigma^2\right)^{\frac{1}{2}} - \frac{1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] + \log \left(\lambda \right) -\lambda\sigma
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto \sum_{n=1}^{N} \left[ - \log \left(\sigma\right) - \frac{1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] + \log \left(\lambda \right) -\lambda\sigma
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto - \sum_{n=1}^{N} \log \left(\sigma\right) - \sum_{n=1}^{N} \frac{1}{2\sigma^2} \left( x_n  - \mu \right)^2 + \log \left(\lambda \right) -\lambda\sigma
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto -N\log \left(\sigma\right) - \sum_{n=1}^{N} \frac{1}{2\sigma^2} \left( x_n  - \mu \right)^2  -\lambda\sigma
$$
### 3b)
...
{
  temp = -N*log(sigma) - (v*N / (2*sigma^2)) - (lambda * sigma)
  return(temp)
}
```
...
tibble::tibble(
  ...
  mutate(log_post = log_post_sigma_unnorm(sigma, N, v, 0.75)) %>% ggplot(mapping = aes(x = sigma, y = log_post)) + geom_line()
```

What do you think?  
**It has one mode and the shape looks like a parabola**
...
$$
$$
\frac{\delta}{\delta \varphi} \left(\sigma \right) = \frac{\delta}{\delta \varphi} \exp \left(\varphi \right)
$$
$$
\frac{\delta}{\delta \varphi} \left(\sigma \right) = \exp \left(\varphi \right)
$$
$$
\log \left( \frac{\delta}{\delta \varphi} \left(\sigma \right) \right) = \log \left(\exp \left(\varphi \right) \right)
$$
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  log_post <- -N*log(sigma) - (v*N / (2*sigma^2)) - (lambda * sigma)
  
  # account for the derivative adjustment
  temp = log_post + varphi
  return(temp)
}
```
...
tibble::tibble(
  x = seq(from = (-5/8)*pi, to = (9/8)*pi, length.out = 1001)
) %>% 
...
opt_sine_01 <- optim( par = 0,
                     ...
                     gr = NULL,
                     method = "Brent",
                     lower = (-5/8)*pi, upper = (9/8)*pi,
                     hessian = TRUE,
                     control = list( fnscale = -1))
opt_sine_01
```

**fnsclae = -1 tells optim to minimize and the gr should be set to null**
...
my_func <- function(x, constant = list(a,m))
{
  -1 * exp((x-constant$a)^constant$m)
}
...
tibble::tibble(
  x = seq(from = -0.5, to = 2, length.out =501)
) %>% 
  ggplot(mapping = aes(x = x, y = my_func(x,my_constants))) + geom_line()
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, y = my_func(x,my_constants))) + geom_line() + geom_vline(xintercept = opt_result$par , color = "red")
```
...
**It remains the same, only the number of iterations the optim required changes**
