...
```{r, solution_01d}
model_pred_df %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(size=3.5, alpha=0.5)
```
...
```{r, solution_01f}
model_pred_df %>% ggplot(mapping = aes(x=x)) + geom_line(mapping = aes(y = pred_prob), color = "red") + geom_point(mapping=aes(y = y), size = 3.5, alpha = 0.5)
```
...
What do you think?  
**It does "follow" the predicted probability as the response is first at 1 and starts curving downwards when the predicted probability is at 0 and then makes its way back up towards 1.**
...
```{r, solution_02b}
table(model_class_0.5$pred_class, model_class_0.5$obs_class)
#Using the accuracy formula = (TP+TN)/(TP+TN+FP+FN)
accuracy_0.5 <- (35+53)/(35+31+6+53)
accuracy_0.5
```
...
**False-Positives are 6, True-Negatives are 53** 
...
```{r, solution_02d}
#Using the formula for sensitivity = TP/(TP+FN)
sensitivity_0.5 <- 35/(35+31)
sensitivity_0.5
```
```{r}
#Using the formula for Specificity = TN/(TN+FP) 
specificity_0.5 <- 53/(53+6)
```
...
```{r, solution_03a}
table(model_class_0.7$pred_class, model_class_0.7$obs_class)
#Using the accuracy formula = (TP+TN)/(TP+TN+FP+FN)
...
```{r, solution_03c}
table(model_class_0.3$pred_class, model_class_0.3$obs_class)
#Using the accuracy formula = (TP+TN)/(TP+TN+FP+FN)
...
```{r, solution_03e}
temp_df <- data.frame(sensitivity = c(sensitivity_0.3, sensitivity_0.5, sensitivity_0.7), fpr = c(fpr_0.3, fpr_0.5, fpr_0.7))
ggplot(temp_df, mapping = aes(fpr)) + geom_point(mapping = aes(y = sensitivity), size = 3) + geom_abline(slope = 1, intercept = 0) + coord_equal(xlim=c(0,1), ylim=c(0,1)) + geom_line(mapping = aes(y = sensitivity))
```
...
ctrl_acc <- trainControl(method = "repeatedcv", number = 10, repeats = 10)
...
### model 1
set.seed(2021)
mod_1_acc <- train(method = "glm", data = df, y ~ x1 + x2 + x3 + x4, trControl = ctrl_acc, metric = metric_acc)
```

```{r, solution_04d_02, eval=TRUE}
### model 2
set.seed(2021)
mod_2_acc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x2 + I(x2^2) + x3 + I(x3^2) + x4 + I(x4^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_03, eval=TRUE}
### model 3
set.seed(2021)
mod_3_acc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x2 + I(x2^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_04, eval=TRUE}
### model 4
set.seed(2021)
mod_4_acc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x3 + I(x3^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_05, eval=TRUE}
### model 5
set.seed(2021)
mod_5_acc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x4 + I(x4^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_06, eval=TRUE}
### model 6
set.seed(2021)
mod_6_acc <- train(method = "glm", data = df, y ~ x2 + I(x2^2) + x3 + I(x3^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_07, eval=TRUE}
### model 7
set.seed(2021)
mod_7_acc <- train(method = "glm", data = df, y ~ x2 + I(x2^2) + x4 + I(x4^2), trControl = ctrl_acc, metric = metric_acc) 
```

```{r, solution_04d_08, eval=TRUE}
### model 8
set.seed(2021)
mod_8_acc <- train(method = "glm", data = df, y ~ x3 + I(x3^2) + x4 + I(x4^2), trControl = ctrl_acc, metric = metric_acc) 
```
...
**model-3 seems to be the best model**
...
**The number of false positives and false negatives increases as accuracy decreases i.e., as model goes from best to worse.**
...
ctrl_roc <- trainControl(method="repeatedcv", number = 10, repeats = 10,
...
### model 1
set.seed(2021)
mod_1_roc <- train(method = "glm", data = df, y ~ x1 + x2 + x3 + x4, trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_02, eval=TRUE}
### model 2
set.seed(2021)
mod_2_roc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x2 + I(x2^2) + x3 + I(x3^2) + x4 + I(x4^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_03, eval=TRUE}
### model 3
set.seed(2021)
mod_3_roc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x2 + I(x2^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_04, eval=TRUE}
### model 4
set.seed(2021)
mod_4_roc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x3 + I(x3^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_05, eval=TRUE}
### model 5
set.seed(2021)
mod_5_roc <- train(method = "glm", data = df, y ~ x1 + I(x1^2) + x4 + I(x4^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_06, eval=TRUE}
### model 6
set.seed(2021)
mod_6_roc <- train(method = "glm", data = df, y ~ x2 + I(x2^2) + x3 + I(x3^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_07, eval=TRUE}
### model 7
set.seed(2021)
mod_7_roc <- train(method = "glm", data = df, y ~ x2 + I(x2^2) + x4 + I(x4^2), trControl = ctrl_roc, metric = metric_roc)
```

```{r, solution_05b_08, eval=TRUE}
### model 8
set.seed(2021)
mod_8_roc <- train(method = "glm", data = df, y ~ x3 + I(x3^2) + x4 + I(x4^2), trControl = ctrl_roc, metric = metric_roc)
```
...
**Highest Sensitivity or TPR - mod-1 & Lowest FPR(1-Specificity) - mod-3** 
...
**When the threshold is lower, the values for 'event' is selected as 1 for all the values below the threshold and that's why the sensitivity remains close to 1**
...
```{r, solution_05g}
mod_1_roc$pred %>% group_by(Resample) %>% roc_curve(obs, event) %>% autoplot() + guides(color = "none")
```
...
**Model-3 is the best model. Yes, the result is consistent. Model-8 is completely ineffective because it swaps errors as the threshold changes, Dropping the threshold provides no benefit in tpr at a fixed fpr.**
...
**The values in the event and non-event as stated are probabilities of event and non-event so the sum of the two columns would always be 1**
...
```{r, solution_06c}
test_df_1_b <- test_df_1 %>% mutate(pred_bin = cut(event, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE))
```
...
```{r, solution_06e}
test_df_1_b %>% ggplot(mapping = aes(x = pred_bin)) + geom_bar()
```
...
**The bin [0.9, 1] contains only events. There are no bins that contain only non-events.**

```{r, solution_06f}
test_df_1_b %>% ggplot(mapping = aes(x = pred_bin)) + geom_bar(mapping = aes(fill = y)) + scale_color_brewer(palette = 'Set1')
```
...
```{r, solution_06g}
test_df_1_b %>% ggplot(mapping = aes(x = pred_bin)) + geom_bar(mapping = aes(fill = y), position = "fill") + scale_color_brewer(palette = 'Set1')
```
...
```{r}
temp <- test_df_1_b%>% count(pred_bin)
bins <- temp$pred_bin[0:10]
```


```{r}
events_1 <- test_df_1_b[test_df_1_b$y == "event",]
event_count_1 <- events_1 %>% count(pred_bin)
total_count_1 <- test_df_1_b %>% count(pred_bin)
event_prop_1 <- event_count_1/total_count_1
event_prop_1$n
```

```{r}
mid_bin <- seq(0 + 0.1/2, 1 - 0.1/2, by = 0.1)
mid_bin
```

```{r}
my_calcurve_1 <- data.frame(pred_bin = bins, prop_event = event_prop_1$n, mid_bin = mid_bin)
my_calcurve_1
```
...
```{r, solution_04d}
test_df_3_b <- test_df_3 %>% mutate(pred_bin = cut(event, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE))
```

```{r}
test_df_8_b <- test_df_8 %>% mutate(pred_bin = cut(event, breaks = seq(0, 1, by = 0.1), include.lowest = TRUE))
```
...
```{r, solution_07e}
test_df_3_b %>% ggplot(mapping = aes(x = pred_bin)) + geom_bar()
```
...
```{r, solution_07f}
test_df_8_b %>% ggplot(mapping = aes(x = pred_bin)) + geom_bar()
```
...
```{r}
events_3 <- test_df_3_b[test_df_3_b$y == "event",]
event_count_3 <- events_3 %>% count(pred_bin)
total_count_3 <- test_df_3_b %>% count(pred_bin)
event_prop_3 <- event_count_3/total_count_3
event_prop_3$n
```

```{r}
my_calcurve_3 <- data.frame(pred_bin = bins, prop_event = event_prop_3$n, mid_bin = mid_bin)
my_calcurve_3
```
...
```{r}
events_8 <- test_df_8_b[test_df_8_b$y == "event",]
event_count_8 <- events_8 %>% count(pred_bin, .drop = FALSE)
total_count_8 <- test_df_8_b %>% count(pred_bin, .drop = FALSE)
event_prop_8 <- event_count_8/total_count_8
event_prop_8$n
```

```{r}
my_calcurve_8 <- data.frame(pred_bin = bins, prop_event = event_prop_8$n, mid_bin = mid_bin)
my_calcurve_8[is.na(my_calcurve_8)] = 0
my_calcurve_8
```
...
What do you think?  
**Model-3 seems to be the most well calibrated. Model-1 & Model-3 seem similar, model-8 was the worse. The value of prop-event should be closer to the 45degree line. **
