...
```{r, solution_01a}
df_low_high %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(size = 3)
```

**Is the relationship linear or non-linear?**  

**The relationship leans more towards non-linear as the values of y first increases with values of x at -2, -1 and then decreases gradually with x at 0, 1, 2.**
...
```{r, solution_01c}
rmse_1 <- modelr::rmse(fit_lm_1, df_low_high)
```

```{r}
rmse_2 <- modelr::rmse(fit_lm_2, df_low_high)
```

```{r}
rmse_3 <- modelr::rmse(fit_lm_3, df_low_high)
```

```{r}
rmse_4 <- modelr::rmse(fit_lm_4, df_low_high)
```

```{r}
rmse_5 <- modelr::rmse(fit_lm_5, df_low_high)
```

```{r}
rmse_6 <- modelr::rmse(fit_lm_6, df_low_high)
```

```{r}
rmse_7 <- modelr::rmse(fit_lm_7, df_low_high)
```

```{r}
rmse_8 <- modelr::rmse(fit_lm_8, df_low_high)
```

```{r}
rmse_9 <- modelr::rmse(fit_lm_9, df_low_high)
```

```{r}
temp_df <- data.frame(x = 1:9, y = c(rmse_1, rmse_2, rmse_3, rmse_4, rmse_5, rmse_6, rmse_7, rmse_8, rmse_9))
```

```{r}
temp_df %>% ggplot(mapping = aes(x, y)) + geom_point() + geom_line() + scale_y_continuous(name = "RMSE", breaks = c(4.5:8)) + scale_x_continuous(name = "Poly Order", breaks = c(1:9))
```
...
```{r, solution_01d}
r_sq_1 <- modelr::rsquare(fit_lm_1, df_low_high)
```

```{r}
r_sq_2 <- modelr::rsquare(fit_lm_2, df_low_high)
```

```{r}
r_sq_3 <- modelr::rsquare(fit_lm_3, df_low_high)
```

```{r}
r_sq_4 <- modelr::rsquare(fit_lm_4, df_low_high)
```

```{r}
r_sq_5 <- modelr::rsquare(fit_lm_5, df_low_high)
```

```{r}
r_sq_6<- modelr::rsquare(fit_lm_6, df_low_high)
```

```{r}
r_sq_7 <- modelr::rsquare(fit_lm_7, df_low_high)
```

```{r}
r_sq_8 <- modelr::rsquare(fit_lm_8, df_low_high)
```

```{r}
r_sq_9 <- modelr::rsquare(fit_lm_9, df_low_high)
```

```{r}
temp_df_2 <- data.frame(x = 1:9, y = c(r_sq_1, r_sq_2, r_sq_3, r_sq_4, r_sq_5, r_sq_6, r_sq_7, r_sq_8, r_sq_9))
```

```{r}
temp_df_2 %>% ggplot(mapping = aes(x, y)) + geom_point() + geom_line() + scale_y_continuous(name = "R-Squared") + scale_x_continuous(name = "Poly Order", breaks = c(1:9))
```
...
**I think the 9th model is the best as the RMSE value for it is the minimum(4.27) and the R_Squared value(0.69) for it is the maximum & yes the two performance metrics agree on the best model as the ideal RMSE should be minimum and R_Squared should be the maximum.**
...
**The coefficient estimates are similar for model 6 & model 9. The level of uncertainty for model 2 is very less, for model 6 is bit more and is the highest for model 9 so it is not consistent across the 3 models rather it is increasing with the model number.**
...
input_viz <- tibble::tibble(x = seq(min(df_low_high$x), max(df_low_high$x), length.out = 201))
```
...
```{r, solution_02d}
df_viz_1 %>% ggplot(mapping = aes(x = x)) + geom_line(mapping = aes(y = pred_trend))
```
...
```{r, solution_02e}
df_viz_1 %>% ggplot(mapping = aes(x = x)) + geom_point(data = df_low_high, mapping = aes(y = y), shape = 0, color = "red") + geom_line(mapping = aes(y = pred_trend), size = 1)
```
...
**Looking at model 9, the trends seem pretty consistent with the training set. The predictive trends for model 9 are fitting quiet correctly.**
...
```{r, solution_03b}
df_low_high %>% ggplot(mapping = aes(x = x)) + geom_point(mapping = aes(y = y), data = df_train, shape = 4, color = "darkslateblue", size = 2) + geom_point(mapping = aes(y = y), data = df_holdout, shape = 1, color = "darkslategrey", size = 2)
```
...
```{r, solution_03d}
rmse_hold_1 <- modelr::rmse(fit_train_1, df_holdout)
```

```{r}
rmse_hold_2 <- modelr::rmse(fit_train_2, df_holdout)
```

```{r}
rmse_hold_3 <- modelr::rmse(fit_train_3, df_holdout)
```

```{r}
rmse_hold_4 <- modelr::rmse(fit_train_4, df_holdout)
```

```{r}
rmse_hold_5 <- modelr::rmse(fit_train_5, df_holdout)
```

```{r}
rmse_hold_6 <- modelr::rmse(fit_train_6, df_holdout)
```

```{r}
rmse_hold_7 <- modelr::rmse(fit_train_7, df_holdout)
```

```{r}
rmse_hold_8 <- modelr::rmse(fit_train_8, df_holdout)
```

```{r}
rmse_hold_9 <- modelr::rmse(fit_train_9, df_holdout)
```

```{r}
temp_df_3 <- data.frame(x = 1:9, y = c(rmse_hold_1, rmse_hold_2, rmse_hold_3, rmse_hold_4, rmse_hold_5, rmse_hold_6, rmse_hold_7, rmse_hold_8, rmse_hold_9))
```

```{r}
temp_df_3 %>% ggplot(mapping = aes(x, y)) + geom_point() + geom_line() + scale_y_continuous(name = "RMSE") + scale_x_continuous(name = "Hold Order", breaks = c(1:9))
```
...
**On the hold-out evaluation, it can be said that the rmse for model 2 is the least which is not the same as previously predicted. It can also be seen that model 9 has the maximum rmse value which makes it the worst model.**
...
**We can see a difference for the cases x^2, x^4, x^6 other than that most of the cases have similar uncertainties **
...
**From the plot, it can be observed that they are quite similar except for x. **
...
**The model will be trained and tested 5 times.**
...
**Rsquared is one of the most common performance metric. **
...
```{r, solution_04e_a}
lowhigh_results %>% summary(metric = my_metric)
```


```{r, solution_04e_b}
lowhigh_results %>% dotplot(metric = my_metric)
```

Which model is the best?  

**Model 6 is the best according to the summary and dotplot **
...
```{r, solution_04f}
coefplot::multiplot(mod_lowhigh_2$finalModel, mod_lowhigh_3$finalModel, mod_lowhigh_6$finalModel)
```

What do you think?  

**The coefficient estimates are not that similar and the level of uncertainty is really consistent across the 3 models.**
...
```{r, solution_05a}
df_low_low %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(size = 3)
```

How does it compare?  

**The points in this graph are much more closer to each other as compared to the first one which was more spread out. **
...
### linear relationship (degree 1)
set.seed(2001)
mod_lowlow_1 <- train(y ~ x, data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_lowlow_2 <- train(y ~ x + I(x^2), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### cubic relationship (degree 3)
set.seed(2001)
mod_lowlow_3 <- train(y ~ x + I(x^2) + I(x^3), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 4
set.seed(2001)
mod_lowlow_4 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 5
set.seed(2001)
mod_lowlow_5 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 6
set.seed(2001)
mod_lowlow_6 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 7
set.seed(2001)
mod_lowlow_7 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 8
set.seed(2001)
mod_lowlow_8 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 9
set.seed(2001)
mod_lowlow_9 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), data = df_low_low, metric = my_metric, trControl = my_ctrl, method = "lm")
...
```{r, solution_05c_a}
lowlow_results %>% summary(metric = my_metric)
```


```{r, solution_05c_b}
lowlow_results %>% dotplot(metric = my_metric)
```

Which model is the best?  

**According to the summary and dotplot, model 5 is the best. **
...
**No, the model for both cases are different. Model 1 remained the same but not the others.** 
...
```{r, solution_06a}
df_high_high %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(size = 3)
```
...
### linear relationship (degree 1)
set.seed(2001)
mod_highhigh_1 <- train(y ~ x, data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_highhigh_2 <- train(y ~ x + I(x^2), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### cubic relationship (degree 3)
set.seed(2001)
mod_highhigh_3 <- train(y ~ x + I(x^2) + I(x^3), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 4
set.seed(2001)
mod_highhigh_4 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 5
set.seed(2001)
mod_highhigh_5 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 6
set.seed(2001)
mod_highhigh_6 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 7
set.seed(2001)
mod_highhigh_7 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 8
set.seed(2001)
mod_highhigh_8 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
### degree 9
set.seed(2001)
mod_highhigh_9 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), data = df_high_high, metric = my_metric, trControl = my_ctrl, method = "lm")
...
```{r, solution_06c_a}
highhigh_results %>% summary(metric = my_metric)
```


```{r, solution_06c_b}
highhigh_results %>% dotplot(metric = my_metric)
```
...
**Yes, the model remained the same. The models that were identified as poor remained the same. **
...
**The model with larger sample size and less noise have more accurate results with less complex behavior. The more the noise the more complex the behavior of the model.**
