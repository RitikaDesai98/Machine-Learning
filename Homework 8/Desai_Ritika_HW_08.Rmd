...
**Based on the visualization, it can be said there is a trend between x2 and the response as the response increases with increase in x2 and decreases after x2 crosses 0. But there doesn't seem to be a trend between x1 and the response.**
...
```{r}
coefplot::coefplot(mod01)
```

**$x_1$ is statistically significant as it always remains positive.**
...
```{r}
coefplot::coefplot(mod02)
```

**There are 4 features: $x_1$ : $x_2$, $x_1$, $x_2$, intercept. $x_1$ : $x_2$, $x_1$, $x_2$ are statistically significant. The intercept isn't**
...
```{r}
coefplot::coefplot(mod03)
```

**There are 8 features: $x_2$, $x_1^2$, $x_2$, $x_2^2$, $x_1$:$x_2$, $x_1$:$x_2^2$, $x_1^2$:$x_2$, $x_1^2$:$x_2^2$. Only the Intercept and $x_2^2$ are statistically significant**
...
```{r, solution_01f}
mod05 <- lm(y ~ (splines::ns(x1, 12)) * (x2 + I(x2^4)), data = df)
```
...
```{r, solution_01h}
train01 <- broom::glance(mod01)
train02 <- broom::glance(mod02)
train03 <- broom::glance(mod03)
train04 <- broom::glance(mod04)
train05 <- broom::glance(mod05)
train06 <- broom::glance(mod06)
```
...
**Model - 6 is the best according to r-squared**
...
**Model - 3 has the least AIC, so it is the best according to AIC**
...
**Model - 3 has the least BIC, so it is the best according to BIC**
...
**Yes, the predictions are consistent with the model performance rankings based on AIC/BIC. Model-6 is the worse according to BIC. The defining characteristic can be the relationship between the size of the confidence interval to the size of the prediction interval. The total uncertainty is dominated by the uncertainty on average. This is because all coefficients are quite uncertain. **
...
X03 <- model.matrix(~ (x1 + I(x1^2))*(x2 + I(x2^2)), data = df)
...
X06 <- model.matrix(~ (splines::ns(x1, 12)) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)), data = df)
...
**Since the value of the ratio is greater than 1 so the numerator is bigger than the denominator and Model 3 is the numerator, hence Model 3 is better than Model 6.**
...
**The coefficient posterior distribution is very small for very strong as compared to strong. The uncertainty on each parameter decreases for very strong compared to strong. There are no statistically significant features for model 3 in very strong but $x_2^2$ is statistically significant for strong. Similarly for model 6, $x_2^4$ is statistically significant for very strong but none are statistically significant for strong.**
...
**The prior does influence which model is considered to be better. For strong, model 3 is better but for very strong model 6 is better.**
...
post_pred_summary_viz_03_weak %>% 
  left_join(viz_grid %>% tibble::rowid_to_column("pred_id"),
            by = 'pred_id') %>% ggplot(mapping = aes(x = x1)) + geom_ribbon(mapping = aes(ymin = y_lwr, ymax = y_upr), fill="orange") + geom_ribbon(mapping = aes(ymin = mu_lwr, ymax = mu_upr), fill = "grey") + geom_line(mapping = aes(y = mu_avg)) + facet_wrap(~x2)
```
...
```{r, solution_04e}
X06_test <- model.matrix(~ (splines::ns(x1, knots = make_splines_training_knots(J = 12, train_data = viz_grid, xname = as.character('x1')))) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)), data = viz_grid)
...
**As the prior standard deviation decreases the model becomes quite uncertain. The confidence interval ranges from 4000 in weak. Yes, for both Model 3 and Model 6 the posterior predictions are consistent with the posterior coefficients.**
...
**For both the models only the intercept and $x_2^2$ have not been turned off. The final tuned model have turned off the same features only the values of the coefficients are different. **
