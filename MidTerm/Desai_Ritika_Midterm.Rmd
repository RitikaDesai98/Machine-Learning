...
**There is 1 prior trial as a is the number of events and b is the number of non-events. The number of trials = events + non-events.**
**Since there is no prior knowledge or understanding, the values of a and b are randomly assumed thus making it uniformed.**
...
**Beta distribution is the posterior for the unknown event probability.**
...
a <- 0.5
b <- 0.5
post_df_focus_from_vague <- df_focus %>% mutate(anew = a + num_events, bnew = b + (num_trials - num_events))
```
...
summary_post_df_focus_from_vague <- post_df_focus_from_vague %>% mutate(post_avg = anew/(anew+bnew), post_q05 = qbeta(0.05, anew, bnew), post_q95 = qbeta(0.95, anew, bnew))
```
...
```{r, solution_01f}
summary_post_df_focus_from_vague %>% ggplot(mapping = aes(x = as.factor(player_id))) + geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) + geom_point(mapping = aes(y = post_avg)) + geom_point(mapping = aes(y=num_events/(num_events+num_trials)), shape = 1, color = "blue")
```

**Yes, both are true. There are players with MLEs that are outside the posterior uncertainty interval as well as there are players with posterior mean that are close to the MLEs.**  
...
```{r, solution_01g}
summary_post_df_focus_from_vague %>% ggplot(mapping = aes(x = as.factor(num_trials))) + geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95, group = player_id), position = position_dodge(0.2)) + geom_point(mapping = aes(y = post_avg, group = player_id), position = position_dodge(0.2)) + geom_point(mapping = aes(y=num_events/(num_events+num_trials), group = player_id), shape = 1, color = 'blue' , position = position_dodge(0.2))
```

**Player 1 has the highest posterior uncertainty followed by player 3.**
...
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto \prod_{j=1}^{J} \left( \left(\mu^{m_j}\left(1 - \mu \right)^{N_j-m_j} \right)\times \left( \mu^{a-1} \left(1 - \mu^{b-1} \right) \right) \right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log \left( \prod_{j=1}^{J} \left( \left(\mu^{m_j}\left(1 - \mu \right)^{N_j-m_j} \right)\times \left( \mu^{a-1} \left(1 - \mu^{b-1} \right) \right) \right) \right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log \left( \prod_{j=1}^{J} \left(\mu^{m_j}\left(1 - \mu \right)^{N_j-m_j} \right)\right) +log\left( \mu^{a-1} \left(1 - \mu^{b-1} \right) \right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto \sum_{j=1}^{J}\left( log\left( \mu^{m_j}\right) + log \left(\left(1-\mu\right)^{N_j-m_j} \right)\right) + log \left(\mu^{a-1} \right) + log \left(\left(1 - \mu \right)^{b-1} \right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto \sum_{j=1}^{J}\left(\left(m_j \right)log\left(\mu \right) + \left(N_j - m_j \right) log \left(1 - \mu \right) \right) + \left(a-1 \right)log\left(\mu \right) + \left(b-1 \right)log\left( 1-\mu\right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log\left(\mu \right)\sum_{j=1}^{J}m_j + log\left(1 - \mu \right)\sum_{j=1}^{J}\left(N_j - m_j \right) + \left(a-1 \right)log\left(\mu \right) + \left(b-1 \right)log\left( 1-\mu\right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log\left(\mu \right) \left(\sum_{j=1}^{J}m_j + \left(a - 1 \right) \right) + log\left(1 - \mu \right)\left(\sum_{j=1}^{J} \left( N_j - m_j\right) + \left(b-1 \right)\right)
$$
...
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log\left(\mu \right)\left(\bar{m}J+\left(a-1 \right) \right) + log\left( 1-\mu\right) \left(\left(\bar{N}J - \bar{m}J \right) + \left(b-1 \right)\right)
$$
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto log\left(\mu \right)\left(\bar{m}J+\left(a-1 \right) \right) + log\left( 1-\mu\right) \left(\left(\bar{N} - \bar{m} \right)J + \left(b-1 \right)\right)
$$
...
$$
$$
b_{new} = b + \left(\bar{N} - \bar{m} \right)J
$$
...
a_new <- a + (mean(df_focus$num_events)*nrow(df_focus))
b_new <- b + ((mean(df_focus$num_trials)) - (mean(df_focus$num_events)))*nrow(df_focus)
post_df_focus_pooled <- df_focus %>% mutate(anew = a_new, bnew = b_new)
```
...
summary_post_df_focus_pooled <- post_df_focus_pooled %>% mutate(post_avg = (anew/(anew+bnew)), post_q05 = qbeta(0.05, anew, bnew), post_q95 = qbeta(0.95, anew, bnew))
```
...
```{r, solution_02f}
summary_post_df_focus_pooled %>% ggplot(mapping = aes(x = as.factor(player_id))) + geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) + geom_point(mapping = aes(y = post_avg)) + geom_point(mapping = aes(y=num_events/(num_events+num_trials)), shape = 1, color = 'blue')
```

What do you think?  
**All the players are outside the posterior uncertainty interval and there are no players close to the MLEs.**
...
**I think that the data has an unequal number of trials for each player. There are players with 1 trial and then there are players with 107 trials. Taking the average of these trials as the prior doesn't make much sense as that would give incorrect results and outputs such as the visualization above.**
...
```{r, solution_03a}
df_all %>% ggplot(mapping = aes(x = num_events/num_trials)) + geom_histogram(binwidth = 0.05) + labs(x = "catch rate")
```
...
```{r, solution_03b}
df_all %>% ggplot(mapping = aes(x = num_events/num_trials)) + geom_histogram(binwidth = 0.05) + facet_wrap(~num_trials > 24) + labs(x = "catch rate")
```

**We can say that the players with greater than 25 targets have a catch rate above 0.5.**
...
log_ab_res_01 <- optim(par = init_guess_01, fn = my_beta_loglik, gr = NULL, info_for_ab, method = "BFGS", hessian = TRUE, control = list(fnscale = -1))
```

Perform the optimization using the second starting guess.  

```{r, solution_03e_c, eval=TRUE}
log_ab_res_02 <- optim(par = init_guess_02, fn = my_beta_loglik, gr = NULL, info_for_ab, method = "BFGS", hessian = TRUE, control = list(fnscale = -1))
```
...
```{r, solution_03g_b}
df_24 %>% ggplot(mapping = aes(x = catch_rate)) + geom_histogram(mapping = aes(y = stat(density)), binwidth = 0.05) + geom_line(data = prior_for_viz, mapping = aes(x = x, y = beta_pdf), color = "red", size = 1.15)
```

**The empirically derived prior distribution on the event probability is equal to the observed histogram of the catch rates**
...
post_df_focus_empbayes <- df_focus %>% mutate(anew = ab_emp_bayes[1]+num_events, bnew = (ab_emp_bayes[2]+(num_trials-num_events)))
```
...
summary_post_df_focus_empbayes <- post_df_focus_empbayes %>% mutate(post_avg = (anew/(anew+bnew)), post_q05 = qbeta(0.05, anew, bnew), post_q95 = qbeta(0.95, anew, bnew))
```
...
```{r, solution_04c}
summary_post_df_focus_empbayes %>% ggplot(mapping = aes(x = as.factor(player_id))) + geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) + geom_point(mapping = aes(y = post_avg)) + geom_point(mapping = aes(y = num_events / num_trials), shape = 1, color = "blue")
```

**The uncertainty interval for many players is reduced in this visualization. The MLEs of the players with higher trials are within the uncertainty interval. The MLEs of players 308, 734, 763, 765, 791, 808, 834, 835 have changed alot. **
...
```{r, solution_04d}
summary_post_df_focus_empbayes %>% ggplot(mapping = aes(x = as.factor(num_trials))) + geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95, group = player_id), position = position_dodge(0.2)) + geom_point(mapping = aes(y = post_avg, group = player_id), position = position_dodge(0.2)) + geom_point(mapping = aes(y = num_events / num_trials, group = player_id), shape = 1, color = "blue", position = position_dodge(0.2))
```
...
```{r, solution_04e}
post_df_all_empbayes <- df_all %>% mutate(anew = ab_emp_bayes[1] + num_events, bnew = (ab_emp_bayes[2] + (num_trials - num_events)))
```
...
```{r, solution_04f}
summary_post_df_all_empbayes <- post_df_all_empbayes %>% mutate(post_avg = (anew/(anew+bnew)), post_q05 = qbeta(0.05, anew, bnew), post_q95 = qbeta(0.95, anew, bnew))
```
...
```{r, solution_04g}
summary_post_df_all_empbayes %>% ggplot(mapping = aes(x = num_events / num_trials, y = post_avg)) + geom_point(mapping = aes(color = num_trials)) + geom_abline(slope = 1, intercept = 0)
```
...
```{r, solution_04h}
summary_post_df_all_empbayes %>% ggplot(mapping = aes(x = num_trials, y = (post_q95 - post_q05))) + geom_point(mapping = aes(color = num_trials))
```
...
**Looking at both the figures for the different sample sizes, a larger sample size gives much better results as compared to a small sample size.** 
...
```{r, solution_05a}
post_player_eval <- summary_post_df_all_empbayes %>% mutate(prob_grt_67 = 1 - pbeta(0.67, anew, bnew))
```
...
```{r}
post_player_eval <- post_player_eval %>% mutate(catch_rate = (num_events/num_trials))
```
...
**The top 10 players are 409, 353, 498, 467, 382, 493, 447, 278, 567, 302. Apart from the prob_grt_67 value, these players have a high number of trials and events. **
...
**The players are 545, 222, 837, 624, 61, 435, 195, 615, 559, 506 and the smallest number of targets is 47**
...
```{r, solution_05d}
post_player_eval %>% arrange(desc(prob_grt_67 > 0.75), num_trials)
```

**The players 701, 81, 88, 308, 41, 606, 264, 321, 376, 476 have smaller number of trials but have prob_grt_67 values greater than 0.75.**
...
**Looking the at the figure for the pooled estimate, it can be said that any player with catch rate >= 0.67 is a good player.**
...
```{r, solution_05f}
post_player_eval %>% ggplot(mapping = aes(x = post_avg, y = (post_q95 - post_q05), label = player_id)) + geom_point(mapping = aes(color = num_trials)) 
```
...
```{r}
post_player_eval %>% ggplot(mapping = aes(x = post_avg, y = (post_q95 - post_q05), label = player_id)) + geom_point(mapping = aes(color = num_trials)) +
geom_text(aes(label = ifelse(num_trials < 100 & post_avg > 0.8 & (post_q95 - post_q05) < 0.2, as.character(player_id), "")),  hjust=0,vjust=0)
```


**A player that has high post_avg, less uncertainty along with less number of trials would be an ideal player to choose. Looking at the above plot, we can choose any player from 831, 795, 418, 473 as they match that criteria** 
